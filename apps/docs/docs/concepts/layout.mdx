...
Both axes share **one spacing step**, measured in pixels, and the grid is
centred on the stage. Two neighbours are therefore the same distance apart
whether they sit side by side or one above the other — and that distance does
not change with the number of nodes. Add a lane and the diagram grows within the
stage rather than redistributing itself across it.
...
- **Components with named terminals.** Alongside the pictograms and shapes,
  ...
  bottom. Such a top/bottom-wired component draws its **label to a side** (left or
  right, toward the outside) instead of below, so the text never sits on the
