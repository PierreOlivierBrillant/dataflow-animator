...
A packet also **waits at its origin long enough to be read**. It carries text — a
header, a query, a row count — and it meets the reader while still standing
still, so that pause is at least as long as its content needs. It is counted once
per packet: the next leg of the same route shows the same text.
