...
| `fallback`   | `ReactNode`                                | —               | Rendu côté serveur et jusqu'au montage du player.                 |
...
:::caution Changement en v3

Ces quatre fonctions prenaient et renvoyaient des nœuds React. Elles pilotent
désormais le registre framework-agnostique par lequel le player rend réellement —
si elles avaient continué de pointer sur celui de React, elles seraient devenues
silencieusement inopérantes. Voir le
...
:::caution Retiré en v3

`useClock` et le type `Clock` ont disparu. L'horloge du player vit désormais dans
le cœur framework-agnostique et n'est plus un hook React : le hook exporté ne
pilotait donc plus rien de ce que fait le player.
