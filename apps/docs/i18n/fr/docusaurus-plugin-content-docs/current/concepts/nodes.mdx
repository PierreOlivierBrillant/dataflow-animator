...
[`direction: 'circuit'`](./layout.mdx#mode-circuit-schémas-électriques), où les
`connections` deviennent des fils orthogonaux. Deux champs les accompagnent :

- `value` + `unit` construisent le libellé (`value: 220, unit: 'Ω'` → `"220 Ω"` ;
  combiné à `text` si les deux sont présents, p. ex. `"R1 · 220 Ω"`) ;
- `closed` (sur un `switch` / `push_button`) fixe l'état initial du contact,
  animé à l'exécution par l'action [`toggle`](../reference/actions.mdx#toggle).
...
[`set_icon`](../reference/actions.mdx#set_icon), allumé par
[`set_color`](../reference/actions.mdx#set_color)) — pour les entrées et sorties
d'un schéma logique.

### Blocs fonctionnels

Certains circuits se lisent mieux quand un sous-circuit est dessiné comme **un
seul boîtier étiqueté** plutôt que comme les portes qu'il contient. Huit types
couvrent les blocs dont le nombre de bornes est fixe :
...
:::note Uniquement les blocs à broches fixes

Un bloc dont le NOMBRE de bornes varie avec sa taille — un registre N bits, un
multiplexeur 4:1, un décodeur n→2ⁿ — n'est délibérément **pas** un type. Un type
par taille multiplierait les symboles sans fin ; ces blocs ont besoin que leurs
broches soient déclarées dans la spec, ce que le format ne fait pas encore.

:::

### Boîtiers génériques : `type: 'block'`

Les huit blocs ci-dessus sont des symboles de la bibliothèque parce que leur
nombre de bornes ne varie jamais. Un multiplexeur 4:1, un décodeur n→2ⁿ, un
registre N bits ou une UAL en ont chacun un différent — un type de nœud par
taille, indéfiniment. `type: 'block'` est la porte de sortie : **vous déclarez
les bornes, et la boîte se dimensionne dessus.**
...
Chaque broche porte un `name` — ce à quoi `"mux:i2"` fait référence —, un `side`
optionnel (`'left'` par défaut, plus `'right'`, `'top'`, `'bottom'`) et un
`label` optionnel imprimé dans la boîte, qui vaut le nom par défaut. Les bornes
d'une même face sont réparties uniformément dans l'**ordre de déclaration** :
réordonner le tableau réordonne les broches.

La boîte grandit avec ce qu'elle contient : plus haute avec la plus chargée des
faces gauche/droite, plus large avec les libellés les plus longs et avec le
désignateur `body`. Rien n'est mesuré depuis le DOM — la taille découle de la
seule déclaration, et c'est pourquoi un fil atterrit toujours exactement sur le
libellé qui lui correspond.
...
:::note Un boîtier ne participe pas au redressement des amorces

La disposition en circuit décale les composants d'une fraction de leur propre
corps pour aligner deux bornes, ce qui ne fonctionne que parce que tous les
symboles de la bibliothèque rendent à une seule et même taille. Un boîtier, non :
il reste où vous le placez et le routeur dessine les décrochages. Donnez-lui un
peu plus de place qu'à une porte.
...
[`set_color`](../reference/actions.mdx#set_color) sur la connexion) l'emporte
...
Appelez-la **une seule fois au démarrage** de l'application (jamais dans un corps
de composant) — voir l'avertissement dans la
[Référence API](../reference/api.mdx).
...
une fenêtre de navigateur, une image (fixe ou [animée](../reference/actions.mdx#image--une-image-fixe-ou-une-séquence-animée)),
un tableau, ou du [HTML riche](../reference/actions.mdx#html--balisage-riche).
