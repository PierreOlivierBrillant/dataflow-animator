...
(une seule entrée pour `not_gate` / `buffer_gate`), et un nœud `signal` — un
...
pas fusionner. La `color` explicite d'un fil (ou un
...
**verrou SR** (deux NAND couplées en croix — un bit de mémoire). Dans les démos
