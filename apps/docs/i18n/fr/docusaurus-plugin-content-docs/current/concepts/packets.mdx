...
<DataFlowPlayer spec={packetsExample} mode="auto" height={360} />
...
Les [types de nœuds en panneau texte](./nodes.mdx) servent aussi de `kind` de
...
Les deux rendent un en-tête au-dessus d'un corps. Choisissez `complex_node` quand
...
Le [subicon](./nodes.mdx) du nœud — le petit badge de coin qui nomme une
